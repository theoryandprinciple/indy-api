...
module.exports = class DeclarationService extends Schmervice.Service {
    ...
    async fillField(pdfDoc, pageNum, fieldName, field, value) {
        ...
        let textWidth = helveticaFont.widthOfTextAtSize(value, size);

        while (textWidth > field.box.width) {

            --size;

            //if size gets too small, just skip the field entirely, I don't know what'd happen with a 0 or -1 here
            if (size < 2) {
                return;
            }

            textWidth = helveticaFont.widthOfTextAtSize(value, size);
        }

        //Form handling is awful.  Place text directly instead
        page.drawText(value, {
            x: field.box.x + 1,
            y: field.box.y + 3, //text seems to land a little low, bump it up off the line
            size,
            font: helveticaFont
        });
    }

    async findAcroFieldByName(pdfDoc, name) {

        const acroFields = await this.getAcroFields(pdfDoc);
    ...
    async getAcroFields(pdfDoc) {
        ...
        const fieldRefs = await pdfDoc.context.lookup(acroForm.get(PDFName.of('Fields')));

        if (!fieldRefs) {
            return [];
        }

        const len = fieldRefs.size();
        const fields = new Array(len);

        for (let i = 0; i < len; ++i) {
            fields[i] = pdfDoc.context.lookup(fieldRefs.get(i), PDFDict);
        }
