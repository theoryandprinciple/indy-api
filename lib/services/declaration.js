...
module.exports = class DeclarationService extends Schmervice.Service {
    ...
    async generate(variables) {
        ...
        // const pages = declaration.getPages();
    ...
    async sign(variables, trx) {
        ...
        // prevent collisions by prepending a uuid
        const filledId = Uuid();
        const filledName = `declaration-${filledId}.pdf`;
        ...
        const date = Moment(variables.date).format('MM/DD/YYYY');
        page.drawText(date, {
            x: 382,
            y: 394,
            size: 14,
